...
const octokit = new Octokit({
  ...
  timeZone: process.env.GITHUB_TIMEZOME,
 ...
 * rfd/0001.mdx (pass "rfd/0001")
 * rfd/0001/index.mdx (pass "rfd/0001")
...
async function downloadMdxFileOrDirectory(
  ...
  if (content) {
    ...
    files = [
      {
        path: safePath(nodePath.join(mdxFileOrDirectory)),
        content,
      },
    ];
...
async function downloadFileBySha(sha: string) {
  const { data } = await octokit.git.getBlob({
    owner: process.env.GITHUB_OWNER ?? "iancleary",
    repo: process.env.GITHUB_REPO ?? "content",
    file_sha: sha,
  });
  //                                lol
